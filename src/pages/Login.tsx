...
export default function Login() {
    ...
    return (
        <div className="min-h-screen flex items-center justify-center p-4 bg-background relative">
            {isLoading && (
                <div className="absolute top-4 left-1/2 -translate-x-1/2 z-20 rounded-full border border-border/60 bg-background/95 px-3 py-1.5 text-xs text-muted-foreground shadow-sm">
                    Checking session...
                </div>
            )}
            <Link href="/">
                <Button variant="ghost" className="absolute top-4 left-4 sm:top-8 sm:left-8 gap-2 z-10">
                    <ArrowLeft size={16} /> Back
                </Button>
            </Link>
            <Card className="w-full max-w-md space-y-8 border-border bg-card">
                <CardHeader>
                ...
                <CardContent className="space-y-4">
                    ...
                    <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 text-sm text-blue-800">
                        <p className="font-semibold mb-1">👤 Existing Users</p>
                        <p>If you signed up using your Google account, then sign in using the same account to continue seamlessly.</p>
                    </div>
                </CardContent>
