  ...
  app.post("/api/users/:id/avatar", requireAuth, uploadLimiter, avatarUpload.single("avatar"), async (req, res, next) => {
    try {
      ...
      if (currentUser?.avatar?.startsWith("https://res.cloudinary.com/")) {
        ...
        deleteFromCloudinary(currentUser.avatar).catch(err =>
...
// BUG #9 FIX: Validate eventDate is in the future (check at request time)
  ...
  app.post("/api/requests", requireVerifiedAuth, requireOnboarding, async (req, res, next) => {
    try {
      ...
      // BUG #4 FIX: Emit socket.io event and verify user is online
      const io = app.get('io');
      if (io) {
        const sockets = await io.in(parsed.data.toUserId).fetchSockets();
        if (sockets.length > 0) {
          io.to(parsed.data.toUserId).emit('notification', { requestId: request.id, fromUserName: req.user!.name });
        }
      }
  ...
  app.post("/api/requests/:id/accept", requireVerifiedAuth, requireOnboarding, async (req, res, next) => {
    try {
      ...
      const { boostPreferencesFromConnection } = await import("./lib/recommendations");
      ...
      // REAL-TIME FIX: Signal chat creation/update to both parties
      // NOTE: Chat is implicitly created from the request ID, so we use requestId as chatId
      const io = app.get('io');
      if (io) {
          io.to(request.fromUserId).emit('chat_updated', { chatId: requestId });
          io.to(request.toUserId).emit('chat_updated', { chatId: requestId });
          // Also signal notification
          io.to(request.fromUserId).emit('notification');
      }
  ...
  app.post("/api/chats/:chatId/messages", requireVerifiedAuth, requireOnboarding, messageLimiter, async (req, res, next) => {
    try {
      ...
      // BUG #7 FIX: Include sender name in WebSocket message
      if (req.app.get('io')) {
        ...
        req.app.get('io').to(chatId).emit('receive_message', enrichedMessage);
      }
  ...
  app.get("/api/recommendations", requireAuth, async (req, res, next) => {
    try {
      ...
      const { getRecommendationBucket } = await import("./lib/recommendations");
  ...
  app.post("/api/contact", optionalAuth, async (req: any, res, next) => {
    try {
      ...
      try {
        const { mailProvider } = await import("./lib/mail");
  ...
  app.patch("/api/admin/reports/:id", requireAdmin, async (req, res, next) => {
    try {
        ...
        const { sendResolutionEmail } = await import("./lib/mail");
        try {
