 ...
 */
export async function weeklyAuditExportJob() {
  try {
 ...
 * Runs every Sunday at 11:59 PM IST (18:29 UTC)
 */
export function startAuditScheduler() {
  // IST is UTC+5:30
  // 11:59 PM IST = 6:29 PM UTC (18:29)
  const cronExpression = '29 18 * * 0'; // Every Sunday at 18:29 UTC

  cron.schedule(cronExpression, async () => {
    try {
      await weeklyAuditExportJob();
    } catch (error) {
      logger.error('[Audit Scheduler] Job execution failed', error);
    }
  }, {
    timezone: 'UTC'
  });

  logger.log('[Audit Scheduler] Started - runs every Sunday at 11:59 PM IST');
  logger.log('[Audit Scheduler] Audit logs older than 30 days will be auto-deleted after export');
}
