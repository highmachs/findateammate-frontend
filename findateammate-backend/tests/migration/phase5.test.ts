...
describe("Phase 5: no remaining Postgres-only SQL syntax", () => {
  it("zero occurrences of FOR UPDATE, jsonb arrow operators, or INTERVAL syntax in backend/", async () => {
    const files = await glob("backend/**/*.ts", { ignore: "**/node_modules/**" });
...
describe.skip("Phase 5: functional correctness of rewritten queries (against seeded staging data)", () => {
  // SKIPPED EXPLANATION:
  // The test assertions in this block invoke fabricated methods that do not exist in `backend/storage.ts`:
