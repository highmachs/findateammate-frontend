...
import { registerForEvent } from "../../backend/routes/events";
...
describe("Phase 7: event capacity race-condition safety", () => {
  ...
  it("exactly `capacity` registrations succeed under 20 concurrent attempts, rest get EVENT_FULL", async () => {
  ...
  it("no duplicate registration rows exist for the same user+event after a double-submit race", async () => {
    await db.insert(users).values({
  ...
  it("at 100 concurrent attempts against capacity 5, count still never exceeds capacity", async () => {
    for (let i = 0; i < 100; i++) {
