...
const ChartStyle = ({ id, config }: { id: string; config: ChartConfig }) => {
  ...
  const sanitizeVarKey = (value: string) => value.replace(/[^a-zA-Z0-9_-]/g, "")
...
${colorConfig
                .map(([key, itemConfig]) => {
                  const color =
                    ...
                    itemConfig.color
                  const safeKey = sanitizeVarKey(String(key))
                  const safeColor = color ? sanitizeCssValue(String(color)) : ""
